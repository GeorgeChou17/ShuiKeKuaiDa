...
| **内存** | 16 GB RAM |
| **磁盘** | SSD，1.5 GB 可用空间 |
| **显卡** | NVIDIA GTX 1060 或更高（CUDA 11.8/12.6/12.9，GPU 加速 OCR 约 15x CPU）<br>⚠️ GPU 模式可能不稳定，推荐使用 CPU 模式 |
| **网络** | 国内宽带（依赖/模型均走国内镜像） |
...
| 模式 | 适用场景 | OCR 耗时 | 稳定性 |
|------|---------|---------|--------|
| **CPU 模式（推荐）** | 所有用户 | 0.5-5s（取决于 CPU） | ✅ 稳定 |
| **GPU 模式** | NVIDIA 显卡 + CUDA 11.8/12.6/12.9 | 50-100ms | ⚠️ 可能闪退 |

> **注意**：
> - 程序默认使用 CPU 模式。GPU 模式可能因 PaddlePaddle 与驱动不兼容导致闪退
> - 如需使用 GPU 模式，请在「LLM 设置」→「OCR 计算设备」中切换
> - 推荐 CUDA 版本：**11.8 / 12.6 / 12.9**（CUDA 13.x 已知容易闪退）
> - 详见[百度飞桨官网](https://www.paddlepaddle.org.cn/)的硬件支持说明
